...
g_type_interface_peek(G_OBJECT_GET_CLASS(self), CppObjectType::get_type()) // Get the interface.
  ...
  {
    gboolean retval = (*base->is_readable)(self);
    return retval;
  }
...
g_type_interface_peek(G_OBJECT_GET_CLASS(self), CppObjectType::get_type()) // Get the interface.
  ...
  {
    gssize retval = (*base->read_nonblocking)(self, buffer, count, error);
    if(*error)
...
{
  GError* gerror = 0;
  gssize retvalue = g_pollable_input_stream_read_nonblocking(gobj(), buffer, count, 0, &(gerror));
  if(gerror)
    ::Glib::Error::throw_exception(gerror);
  return retvalue;
}
