...
public:
   ...
   * or <tt>0</tt> on error. You must free each of the records and the list when you are
   * done with it. (You can use Glib::list_free_full() with Glib::variant_unref() to do this.).
   */
  ...
  /** Retrieves the result of a previous call to
   * g_resolver_lookup_records_async(). Returns a list of records as Variant
   * tuples. See ResolverRecordType for information on what the records contain.
   * 
   * If the DNS resolution failed, @a error (if non-<tt>0</tt>) will be set to
   * a value from ResolverError. If the operation was cancelled,
   *  @a error will be set to IO_ERROR_CANCELLED.
